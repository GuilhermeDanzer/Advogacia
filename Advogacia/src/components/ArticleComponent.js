...
const ArticleComponent = ({ admin, setValues, values, array, funcaoGet, funcaoDelete }) => {
    ...
    <div className='divContainer'>
      ...
      {array.length < 1 ?
        <div style={{ height: 500, display: 'flex', flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          ...
          />
        </div> : array.map((artigo, i) => {
          const { title, resume, link, _id } = artigo
          return (
        
              
              <div style={{padding:50}} key={i}>
                {admin ? <div style={{ justifyContent: 'flex-end', display: 'flex', marginBottom: 10 }}><EditIcon className='icones' onClick={() => setValues({ title, resume, link, _id, checkedB: true, edit: true })} /><DeleteIcon onClick={() => { deletar(title, _id,) }} className='icones' /></div> : null}
                <h4>{artigo.title}</h4>
                <p>{artigo.resume}</p>
